...
for j=1:size(Theta1NoBias,1)
  for k=1:size(Theta1NoBias,2)
    hidden_layer_reg = hidden_layer_reg + Theta1NoBias(j,k)^2;
  end
end

for j=1:size(Theta2NoBias,1)
  for k=1:size(Theta2NoBias,2)
    output_layer_reg = output_layer_reg + Theta2NoBias(j,k)^2;
  end
end
...
reg = (lambda * (hidden_layer_reg + output_layer_reg) / (2*m) );
J += reg
%
...
for t = 1:m
  % Step 1 - Input Values
  a1 = [1; transpose(X(t, :))]; % Including Bias
  z2 = Theta1 * a1;
